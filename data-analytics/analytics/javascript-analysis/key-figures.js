...
function getHistoricalPERatios(stocksData, keyFigures, balanceSheet) {
  ...
  for (const keyFigure of keyFiguresArray) {
    ...
    if (
      currDate >= fiveYearsAgo &&
      currDate <= toDate &&
      currDate.getMonth() === 11 &&
      currDate.getDate() === 31
    ) {
      const latestDateOfAvailablePrice =
        years[currDate.getFullYear()][symbol][0];
      const latestPrice =
        stocksData[symbol][latestDateOfAvailablePrice]["4. close"];
...
function getBookValueForYear(balanceSheet, date) {
  ...
  for (const report of balanceSheet.annualReports) {
    if (
      date.getFullYear() === new Date(report.fiscalDateEnding).getFullYear()
    ) {
      bookValue =
        report.totalShareholderEquity / report.commonStockSharesOutstanding;
      break;
    }
  }
  return bookValue;
