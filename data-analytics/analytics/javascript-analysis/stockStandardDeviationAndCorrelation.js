...
//needs daily data
function standardDeviationAndCorrelation(
  portfolio,
  stocksData,
  namesToSymbols
) {
    ...
    for (i = numDays - 2; i >= 0; i--) {
      if (
        ...
        const yesterday = Number(
          stocksData[namesToSymbols[stock.name]][usedDates[i + 1]]["4. close"]
        );
        const today = Number(
  ...
  let volatility = {};
  portfolio.securities.forEach((stock) => {
    volatility[stock.symbol] =
      stats.stdev(valuesOfStock[stock.name]) * Math.sqrt(252);
  });
...
function sharpeRatioStocks(portfolio, stocksData, namesToSymbols) {
  ...
  const volatility = standardDeviationAndCorrelation(
    portfolio,
    stocksData,
    namesToSymbols
  ).volatility;
  ...
  let sharpeRatio = {};
  portfolio.securities.forEach((stock) => {
    sharpeRatio[stock.name] =
      (returnOfStocks[stock.name] - riskFreeRate) / volatility[stock.name];
  });
  return {
    sharpeRatioPerSymbol: sharpeRatio,
    portfolioSharpeRatio: backtesting.sharpeRatio(
      portfolio,
      stocksData,
      namesToSymbols
    ),
