...
router.get("/backtest/:id", async (req, res) => {
  ...
  const currSymbols = portfolioFetcher.extractSymbolsFromPortfolio(
    currPortfolio
  );
  const currStocksData = await stockTimeSeries.getStocksDataForSymbols(
    currSymbols
  );
  if (!currStocksData) {
  ...
  const analyzedData = analytics.backtest(
    currPortfolio,
    currStocksData,
    fromDate,
    toDate
...
router.get("/diversification/:id", async (req, res) => {
  ...
  const currCompanyOverviews = await companyOverviews.getCompanyOverviewForSymbols(
    currSymbols
  );
  if (!currCompanyOverviews) {
    response.error =
      "No company overview data for all the stocks in the given portfolio.";
    return res.status(404).json(response);
  }

  const analyzedData = analytics.calculateDiversification(
    currPortfolio,
...
router.get("/dividends/:id", async (req, res) => {
  ...
  const analyzedData = analytics.calculateDividendYields(
    currPortfolio,
    currCompanyOverviews
  );
  response.success = analyzedData;
...
router.get("/peratios/:id", async (req, res) => {
  ...
  const analyzedData = analytics.calculatePERatios(
    currPortfolio,
    currCompanyOverviews
  );
  response.success = analyzedData;
...
router.get("/gainLoss/:id", async (req, res) => {
  ...
  const analyzedData = analytics.calculateGainAndLoss(
    currPortfolio,
    currStocksData
  );
  response.success = analyzedData;
...
router.get("/volatilityCorrelation/:id", async (req, res) => {
  ...
  const analyzedData = analytics.calculateSDAndCorrelationAndVolatility(
    currPortfolio,
    currStocksData
  );
  response.success = analyzedData;
...
router.get("/sharpeRatio/:id", async (req, res) => {
  ...
  const analyzedData = analytics.calculateSharpeRatio(
    currPortfolio,
    currStocksData
  );
  response.success = analyzedData;
...
router.get("/debtEquity/:id", async (req, res) => {
  ...
  const currBalanceSheetPerSymbol = await balanceSheets.getBalanceSheetForSymbols(
    currSymbols
  );
...
router.get("/risk/:symbol", async (req, res) => {
  ...
  const currPortfolio = await portfolioGenerator.generatePortfolioForSymbol(
    symbol
  );
  ...
  result.volatility =
    analyzedData.volatility[Object.keys(analyzedData.volatility)[0]];
...
router.get("/keyfigures/:symbol", async (req, res) => {
  ...
  const result = analytics.calculateKeyFigures(
    currStocksData,
    keyFigureData,
    currBalanceSheetPerSymbol[symbol],
    fiveYearsAgo,
    toDate
  );
  response.success = result;
...
router.get("/treynorRatio/:id", async (req, res) => {
  ...
  const currCompanyOverviews = await companyOverviews.getCompanyOverviewsBySymbolsWithoutReformatting(
    currSymbols
  );
  if (!currCompanyOverviews) {
