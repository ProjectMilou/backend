...
async function convertToEur(amountOfMoney, currencyFrom) {

    if (!!amountOfMoney) {
        let exchangeRate = await getExhangeRateToEur(currencyFrom);
...
async function getExchangeRates() {
    ...
    await fetch(url)
        ...
        .then(data => {
            let exchangeRate = exchangeRateModel.findOneAndUpdate(
                {base: data['base']},
                {
                    $set:
                        {
...
async function getBalanceSheet(symbol, api_key) {
    ...
    await fetch(url)
        ...
        .then(async data => {
            ...
            let exchangeRate = await getExhangeRateToEur(data["annualReports"][0]["reportedCurrency"]);

            annualReportsInEur = [];
            let annualReportsInOtherCurrency = data['annualReports'];
            !!annualReportsInOtherCurrency && annualReportsInOtherCurrency.forEach(annualReport => {
                annualReportsInEur.push(
                    {
                        ...
                        "cashAndCashEquivalentsAtCarryingValue": Math.floor(annualReport['cashAndCashEquivalentsAtCarryingValue'] * exchangeRate),
                        ...
                        "accumulatedDepreciationAmortizationPPE": Math.floor(annualReport['accumulatedDepreciationAmortizationPPE'] * exchangeRate),
...
async function getCashFlow(symbol, api_key) {
    ...
    await fetch(url)
        ...
        .then(async data => {
            let annualReportsInOtherCurrency = data['annualReports'];
            let exchangeRate = await getExhangeRateToEur(annualReportsInOtherCurrency[0]['reportedCurrency']);
            !!annualReportsInOtherCurrency && annualReportsInOtherCurrency.forEach(async annualReport => {
                ...
                let proceedsFromIssuanceOfLongTermDebtAndCapitalSecuritiesNet = (annualReport['proceedsFromIssuanceOfLongTermDebtAndCapitalSecuritiesNet'] * exchangeRate);
                ...
                annualReportsInEur.push(
                    {
                        ...
                        "paymentsForOperatingActivities": Math.floor(paymentsForOperatingActivities),
                        "proceedsFromOperatingActivities": Math.floor(proceedsFromOperatingActivities),
                        "changeInOperatingLiabilities": Math.floor(changeInOperatingLiabilities),
                        "changeInOperatingAssets": Math.floor(changeInOperatingAssets),
                        "depreciationDepletionAndAmortization": Math.floor(depreciationDepletionAndAmortization),
                        ...
                        "proceedsFromRepaymentsOfShortTermDebt": Math.floor(proceedsFromRepaymentsOfShortTermDebt),
                        "paymentsForRepurchaseOfCommonStock": Math.floor(paymentsForRepurchaseOfCommonStock),
                        "paymentsForRepurchaseOfEquity": Math.floor(paymentsForRepurchaseOfEquity),
                        "paymentsForRepurchaseOfPreferredStock": Math.floor(paymentsForRepurchaseOfPreferredStock),
                        "dividendPayout": Math.floor(dividendPayout),
                        "dividendPayoutCommonStock": Math.floor(dividendPayoutCommonStock),
                        "dividendPayoutPreferredStock": Math.floor(dividendPayoutPreferredStock),
                        "proceedsFromIssuanceOfCommonStock": Math.floor(proceedsFromIssuanceOfCommonStock),
                        "proceedsFromIssuanceOfLongTermDebtAndCapitalSecuritiesNet": Math.floor(proceedsFromIssuanceOfLongTermDebtAndCapitalSecuritiesNet),
                        "proceedsFromIssuanceOfPreferredStock": Math.floor(proceedsFromIssuanceOfPreferredStock),
                        "proceedsFromRepurchaseOfEquity": Math.floor(proceedsFromRepurchaseOfEquity),
                        "proceedsFromSaleOfTreasuryStock": Math.floor(proceedsFromSaleOfTreasuryStock),
                        "changeInCashAndCashEquivalents": Math.floor(changeInCashAndCashEquivalents),
...
module.exports.getExhangeRateToEur = getExhangeRateToEur
module.exports.convertToEur = convertToEur