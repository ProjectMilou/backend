...
const getExchangeRate = async (from_currency, to_currency) => {
  ...
  if (from_currency != to_currency && from_currency !== undefined) {
    ...
    try {
      ...
      if (json_response.rates) rate = json_response.rates[to_currency];
      else rate = 1;
    } catch (err) {
...
async function updateStock(position, virtual) {
  ...
  if (stock) {
    ...
    if (virtual) {
      ...
      //convert to EUR
      position.stock.price = await toEur(
        position.stock.price,
        position.stock.marketValueCurrency
      );
    ...
    position.stock.perf7dPercent = percent(
      position.stock.perf7d,
      position.stock.price * position.qty
    );
    position.stock.perf1yPercent = percent(
      position.stock.perf1y,
      position.stock.price * position.qty
    );
    //volatility and debt equity is done in updatePortfolio()
    var detailedAnalysis = await stockDetailedAnalysisModel.findOne({
      symbol: stock.symbol,
    });
    if (detailedAnalysis) {
      var score = 50;
      score +=
        ((detailedAnalysis.averageGoal - position.stock.price) /
          position.stock.price) *
        200;
    ...
    if (virtual) {
      position.totalReturn = returnValueIfDefined(
        position.stock.quote * position.qty -
          position.stock.entryQuote * position.qty
      );
    }
    position.totalReturnPercent = percent(
...
async function updatePortfolio(portfolio) {
  ...
  if (overview.positionCount == 0) {
    ...
    overview.value = stockArrayWithPriceInEur
      .map(({ stock: { price: price }, qty: qty }) =>
        returnValueIfDefined(price * qty)
      )
      .reduce((a, b) => a + b, 0);

    if (overview.value) {
      overview.score =
        stockArrayWithPriceInEur
          .map(({ stock: { price: price, score: score }, qty: qty }) =>
            returnValueIfDefined(price * qty * score)
          )
          .reduce((a, b) => a + b, 0) / overview.value;
    } else {
      overview.score = 0;
    }
    overview.perf7d = stockArrayWithPriceInEur
      .map(({ stock: { perf7d: performance } }) =>
        returnValueIfDefined(performance)
      )
      .reduce((a, b) => a + b, 0);
    overview.perf1y = stockArrayWithPriceInEur
      .map(({ stock: { perf1y: performance } }) =>
        returnValueIfDefined(performance)
      )
      .reduce((a, b) => a + b, 0);
    ...
    var currSymbols = portfolioFetcher.extractSymbolsFromPortfolio(
      currPortfolio
    );
    var currCompanyOverviews;
    if (currSymbols)
      currCompanyOverviews = await companyOverviews.getCompanyOverviewForSymbols(
        currSymbols
      );
    if (currCompanyOverviews) {
      var portfDiversification = diversification.calculateDiversification(
        currPortfolio,
        currCompanyOverviews
      );
    ...
    var currStocksData;
    if (currSymbols)
      currStocksData = await stockTimeSeries.getStocksDataForSymbols(
        currSymbols
      );
    //SD and correlation and volatility
    var SDandCorr;
    if (currStocksData) {
      SDandCorr = analytics.calculateSDAndCorrelationAndVolatility(
        currPortfolio,
        currStocksData
      );
    } else {
    ...
    portfolio.portfolio.positions.forEach((position) => {
      position.stock.volatility = returnValueIfDefined(
        SDandCorr.volatility[position.stock.symbol]
      );
    });
    ...
    if (currStocksData) {
      try {
        analyzedData = analytics.calculateSharpeRatio(
          currPortfolio,
          currStocksData
        );
    ...
    if (currSymbols) {
      currBalanceSheetPerSymbol = await balanceSheets.getBalanceSheetForSymbols(
        currSymbols
      );
    }
    if (currBalanceSheetPerSymbol) {
      const analyzedData = analytics.calculateDebtEquity(
        currPortfolio,
        currBalanceSheetPerSymbol
      );
      portfolio.portfolio.positions.forEach((position) => {
        position.stock.debtEquity = returnValueIfDefined(
          analyzedData.debtEquityPerStock[position.stock.symbol]
        );
      });
    ...
    if (currSymbols)
      currCompanyOverviews2 = await companyOverviews.getCompanyOverviewsBySymbolsWithoutReformatting(
        currSymbols
      );
    if (currStocksData && currCompanyOverviews2) {
      try {
        const analyzedData = analytics.calculateTreynorRatio(
          currPortfolio,
          currStocksData,
          currCompanyOverviews2
        );
    ...
    //others
    portfolio.portfolio.totalReturn = stockArrayWithPriceInEur
      .map(({ totalReturn: performance }) => returnValueIfDefined(performance))
      .reduce((a, b) => a + b, 0);
    portfolio.portfolio.totalReturnPercent = percent(
    ...
    if (
      ...
      var keyFigures = [];
      //find values
      for (let i = 0; i < portfolio.portfolio.positions.length; i++) {
        var position = portfolio.portfolio.positions[i];
        if (!position.stock.missingData) {
          //keyFigures
          let keyFigureData = await KeyFigure.findOne({
          ...
          const result = analytics.calculateKeyFigures(
            currStocksData,
            keyFigureData,
            currBalanceSheetPerSymbol[position.stock.symbol],
            fiveYearsAgo,
            toDate
          );
          //weigh the result
          var weight =
            (position.stock.price * position.stock.qty) /
            portfolio.portfolio.overview.value;
          if (result.length > 0) {
            result.map(
              ({
                date: date,
      ...
      for (let i = 0; i < keyFigures.length; i++) {
        //dividends
        const dataPointDividend = await dividendModel.findOne(
          { symbol: keyFigures[i].symbol },
          {
            symbol: false,
            _id: false,
          }
        ...
        if (
          (!nextDividend || nextDividendOfThisStock < nextDividend) &&
          dataPointDividend &&
          !isNaN(nextDividendOfThisStock)
        ) {
          nextDividend = nextDividendOfThisStock;
        }
        for (var j = 0; j < keyFigures[i].length; j++) {
          if (keyFiguresMappedToDate[keyFigures[i][j].date]) {
            let keyFigure = keyFiguresMappedToDate[keyFigures[i][j].date];
            ...
            if (dataPointDividend) {
              let dataPointOfThisYear = dataPointDividend.dataPoints.find(
                (dataPoint) => {
                  return (
                    dataPoint.date.slice(0, 4) ==
                    keyFigures[i][j].date.slice(0, 4)
                  );
                }
              );
              if (dataPointOfThisYear) {
                if (
                  dataPointOfThisYear.div &&
                  !isNaN(dataPointOfThisYear.div)
                ) {
                  keyFigure.div =
                    returnValueIfDefined(keyFigure.div) +
                    parseFloat(dataPointOfThisYear.div);
                  keyFigure.countHowManyValidDivs++;
                }
                if (dataPointDividend.quota) {
                  keyFigure.dividendPayoutRatio =
                    returnValueIfDefined(keyFigure.dividendPayoutRatio) +
                    parseFloat(returnValueIfDefined(dataPointDividend.quota));
                  keyFigure.countHowManyValidDPR++;
                }
              }
            }
          } else {
            let keyFigure = { year: keyFigures[i][j].date.slice(0, 4) };
            ...
            if (dataPointDividend) {
              ...
              if (dataPointOfThisYear) {
                ...
                if (dataPointDividend.quota) {
                  keyFigure.dividendPayoutRatio = parseFloat(
                    returnValueIfDefined(dataPointDividend.quota)
                  );
                  keyFigure.countHowManyValidDPR++;
                }
              }
...
async function updatePortfolioWhenModified(portfolio) {
  ...
  for (var i = 0; i < positions.length; i++) {
    await updateStockWhenModifed(
      positions[i],
      portfolio.portfolio.overview.virtual
    );
  }
