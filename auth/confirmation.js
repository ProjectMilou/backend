...
const endConfirmationProcess = async (id, token) => {
  const userId = id;
  const databaseToken = await UserTokenModel.findOne({
    userID: userId,
    token: token,
    tokenType: "EMAIL_CONFIRMATION",
  });
...
const resetConfirm = async (id, token) => {
  ...
  else if (userToken.token !== token) return false;
  else if (userToken.expirationDate < new Date().getTime()) return false;
  else {
    const newToken = await randomToken();
    await UserTokenModel.updateOne(
      { userID: id, tokenType: "PASSWORD_RESET" },
      { token: newToken },
      null
    );
...
const endResetProcess = async (id, token) => {
  ...
  else if (databaseToken.expirationDate < new Date().getTime()) return false;
  else {
    await UserTokenModel.deleteMany({
      userID: id,
      tokenType: "PASSWORD_RESET",
